...
const ZonasList = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/zonas/${editingId}`, { nombre });
      } else {
        await api.post('/zonas', { nombre });
      }
      setNombre('');
      setEditingId(null);
      ...
      setError(err.response?.data?.error || 'Error al guardar zona');
    }
  };
  ...
  return (
    <div>
      <div style={styles.header}>
        <h3 style={styles.subtitle}>️ Gestión de Zonas</h3>
      ...
      {loading ? (
        ...
        <div style={styles.emptyState}>No hay zonas registradas</div>
      ) : (
        <div style={styles.tableContainer}>
          <table style={styles.table}>
            ...
            <tbody>
              {zonas.map((zona) => (
                <tr key={zona.id} style={styles.tr}>
                  <td style={styles.td}>{zona.id}</td>
                  <td style={styles.td}><strong>{zona.nombre}</strong></td>
                  <td style={styles.td}>
                    <div style={styles.actions}>
                      <button onClick={() => handleEdit(zona)} style={styles.editButton}>✏️</button>
                      <button onClick={() => handleDelete(zona.id)} style={styles.deleteButton}>️🗑️</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {isModalOpen && (
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            <h3 style={styles.modalTitle}>{editingId ? '✏️ Editar Zona' : '️ Nueva Zona'}</h3>
